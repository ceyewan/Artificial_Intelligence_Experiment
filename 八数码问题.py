...
# 目标状态
end_data = np.array([[1, 8, 7], [2, 0, 6], [3, 4, 5]])
...
def init():
    ...
    while sign:
        ...
        temp = input("请输入初始状态（如:1 2 3 4 5 6 7 8 0）:")
        List1 = temp.split(" ")
    ...
    while sign:
        ...
        List1 = []
        judge = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        temp = input("请输入目标状态（如:1 2 3 4 5 6 7 8 0）:")
        List1 = temp.split(" ")
        for i, value in enumerate(List1):
            List1[i] = int(value)
            if (List1[i] >= 0 and List1[i] <= 8):
        ...
        for i in range(9):
            if judge[i] != 1:
                ...
                print("输入错误!")
                break
    end_data = np.array([[List1[0], List1[1], List1[2]], [
        List1[3], List1[4], List1[5]], [List1[6], List1[7], List1[8]]])
    return start_data
